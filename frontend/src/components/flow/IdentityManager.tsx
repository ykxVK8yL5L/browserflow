...
const IdentityManager = ({ open, onClose }: IdentityManagerProps) => {
    ...
    const handleConfirmDelete = async () => {
        ...
        await handleDelete(deleteDialog.identityId);
                                    ...
                                    identities.map((id) => (
                                        <div key={id.id} className="flex items-center justify-between p-3 rounded-lg border border-border bg-secondary/20 hover:bg-secondary/40 transition-colors group">
                                            ...
                                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                ...
                                                <button onClick={() => requestDelete(id)} className="p-1.5 rounded-md hover:bg-secondary text-muted-foreground hover:text-destructive transition-colors" title="Delete">
                                                    <Trash2 size={14} />
                                                </button>
                                            </div>
                                        </div>
            ...
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            {deleteDialog.open
                                ? `确定要删除 Identity“${deleteDialog.identityName}”吗？相关文件也会被一并移除，且此操作不可撤销。`
